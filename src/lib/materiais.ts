...
export function renderEmCanvas(canvas: HTMLCanvasElement, formato: FormatoMaterial, dados: DadosMaterial, template?: TemplateFeed) {
  ...
  if (!ctx) return;
  renderMaterial(ctx, formato, dados, template);
}
...
export function baixarImagem(formato: FormatoMaterial, dados: DadosMaterial, template?: TemplateFeed) {
  const { W, H } = dimensoes(formato);
...
function canvasPara(formato: FormatoMaterial, dados: DadosMaterial): HTMLCanvasElement {
  ...
  canvas.height = H;
  const ctx = canvas.getContext("2d");
  if (ctx) renderMaterial(ctx, formato, dados);
  return canvas;
}
...
export async function baixarPdf(dados: DadosMaterial, itens: Contemplacao[]) {
  ...
  doc.text("LB REPRESENTAÇÕES", 14, 15);
  doc.setTextColor(...WHITE).setFont("helvetica", "normal").setFontSize(10);
  doc.text(`Resultados oficiais · Sergipe · ${dados.periodo}`, 14, 23);
...
<style>
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .band .lb { width:52px;height:52px;border:2px solid #D4A72C;border-radius:12px;display:flex;align-items:center;justify-content:center;font-weight:800;color:#F4D67B;font-size:22px; }
